...
Joy::Joy() {
  ...
  pnh.param("max_thrust", max_.thrust, 5.5*6*9.8);  // [N]
...
void Joy::JoyCallback(const sensor_msgs::JoyConstPtr& msg) {
  ...
  control_msg_.yaw_rate = msg->axes[buttons_.yaw_left]*max_.rate_yaw;
  ...
  if (is_fixed_wing_) {
    ...
    control_msg_.thrust.z = (axes_.thrust_direction * msg->axes[axes_.thrust] + 1) * max_.thrust / 2.0 ;
  }
